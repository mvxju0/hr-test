window.HR_DATA = (() => {
  ...
  const storeLeaders = {
    가홈: '김민성', 강서: '이선재', 가터: '전다영', 가아: '강민호', 율칸: '김수만', 율롯: '강석주',
    사창: '차명호', 두진: '조민제', 중흥: '윤종우', 시장: '김시원', 복대: '정미정', 강터: '유현우',
    청대: '최준희', 송절: '홍인기', 비하: '유지연', 가경: '정인태', 금광: '김사랑', 용암: '김민서',
    산남: '김민지', 동남: '이승효', 분평: '김재환', 우암: '김성호', 용롯: '이가은', 금천: '김택민',
    지웰: '김지하', 충주: '정희찬', 연수: '김요한', 용산: '이정혁', 호암: '권지수', 증평: '최혁준', 만수: '임민우'
  };
  ...
  stores.forEach((s, i) => {
    ...
    employees.push({
      ...
      status: '활동',
      history: [
        { type: '직급변경', editor: '박근우', before: 'SP2', after: '점장', date: `2025-0${(i % 9) + 1}-12` }
      ]
    });
    ...
    for (let n = 0; n < spCount; n++) {
      ...
      const grade = grades[Math.floor(rand(id) * grades.length)];
      employees.push({
        empNo: `E${empSeq++}`,
        name: spName,
        role: '플래너',
  ...
  const onoff = employees.slice(0, 80).map((e, i) => ({
    empNo: e.empNo,
    name: e.name,
    on: `0${8 + (i % 2)}:${i % 2 ? '10' : '55'}`,
    off: `1${8 + (i % 3)}:${i % 2 ? '10' : '35'}`,
    status: i % 9 === 0 ? '오픈지연' : i % 8 === 0 ? '프리데이' : i % 5 === 0 ? 'OFF' : 'ON',
    note: i % 9 === 0 ? '교통지연 보고' : i % 8 === 0 ? '사전 승인된 프리데이' : '-',
    editHistory: `수정자:${i % 2 ? '박근우' : '유종호'} / ${2026}-03-${String(10 + (i % 20)).padStart(2, '0')}`
  }));

  const approvals = {
    signup: [
      { id: 'S-100', target: '한예린', request: '신규 계정 생성', requester: '정연우', status: '승인 대기' },
      { id: 'S-101', target: '문도윤', request: '신규 계정 생성', requester: '원준기', status: '승인 대기' }
    ],
    employee: [
      { id: 'E-901', target: '김민서', request: '연락처 변경', before: '010-2222-1111', after: '010-8841-3392', requester: '김민서', status: '승인 대기' },
      { id: 'E-902', target: '최준희', request: '소속변경', before: '청대', after: '송절', requester: '원준기', status: '승인 대기' }
    ],
    store: [
      { id: 'P-410', target: '가홈', request: '운영 메모 수정', before: '기본 운영', after: '주말 인력 증원', requester: '김민성', status: '승인 대기' },
      { id: 'P-411', target: '충주', request: '실적 목표 조정', before: '월 850', after: '월 930', requester: '정희찬', status: '승인 대기' }
    ]
  };
  ...
  return { stores, storeLeaders, employees, onoff, approvals, officeMembers, performance };
})();
